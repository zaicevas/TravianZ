...
class Message
{
    ...
    /*
     * ================================================================
     * ROUND DATES AND RULES
     * ================================================================
     *
     * This server runs fixed-length artifact-race rounds, so the message
     * describes the round settings (RoundControl) instead of a World Wonder
    ...
    $window = RoundControl::windowEnabled()
        ? 'every day <b>' . RoundControl::windowLabel() . '</b> (server time, ' . date_default_timezone_get() . '). Outside the window the game pages are closed, but the world keeps running: troops march, buildings finish and resources are produced'
        : 'the game is open around the clock';

    $startGold = (defined('NEW_FUNCTION_REGISTRATION_GOLD') && NEW_FUNCTION_REGISTRATION_GOLD)
        ? (int) NEW_FUNCTION_REGISTRATION_GOLD_VALUE
        : 0;

    $weeklyGold = (int) RoundControl::get('weekly_gold');
    $weeklyGoldText = $weeklyGold > 0
        ? 'every 7 days (from the round start) every player who played during the previous week receives <b>' . $weeklyGold . ' gold</b>'
        : 'there is no weekly gold bonus';

    $quadrant = RoundControl::fixedQuadrant();
    $quadrantText = $quadrant
        ? 'All players start in the <b>' . RoundControl::QUADRANTS[$quadrant] . '</b> region.'
        : '';
    ...
    $welcomemsg = str_replace(
        ...
        [
            ...
            $quadrantText,
            $window,
            $formatDate(RoundControl::artifactsDate()),
            (int) RoundControl::get('round_days'),
            $formatDate(RoundControl::roundEnd()),
            $startGold,
            $weeklyGoldText
        ],
        $welcomemsg
